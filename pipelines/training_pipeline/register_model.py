# pipelines/training_pipeline/register_model.py
import mlflow
from mlflow.tracking import MlflowClient
from config.config_loader import mlflow_config


class ModelRegistry:
    """Compares all runs, selects best model, and promotes it to Production."""

    def __init__(self):
        self.ml_cfg = mlflow_config()
        mlflow.set_tracking_uri(self.ml_cfg["tracking_uri"])
        self.client = MlflowClient()

    def _get_experiment(self):
        experiment = self.client.get_experiment_by_name(self.ml_cfg["experiment_name"])
        if experiment is None:
            raise ValueError(f"Experiment '{self.ml_cfg['experiment_name']}' not found.")
        return experiment

    def _fetch_distilbert_runs(self, experiment_id: str):
        return self.client.search_runs(
            experiment_ids=[experiment_id],
            filter_string ="tags.mlflow.runName LIKE 'distilbert-finetuned%'",
            order_by      =["metrics.f1_score DESC"],
        )

    def _fetch_baseline_runs(self, experiment_id: str):
        return self.client.search_runs(
            experiment_ids=[experiment_id],
            filter_string ="tags.mlflow.runName LIKE 'baseline%'",
            order_by      =["metrics.f1_score DESC"],
        )

    def _print_comparison(self, runs, baseline_runs):
        baseline_f1 = baseline_runs[0].data.metrics.get("f1_score", 0) if baseline_runs else 0
        print("\n" + "="*55)
        print("  MODEL COMPARISON (sorted by F1)")
        print("="*55)
        for run in runs:
            name   = run.data.tags.get("mlflow.runName", run.info.run_id)
            f1     = run.data.metrics.get("f1_score", 0)
            marker = " <- BEST" if run == runs[0] else ""
            print(f"  {name:<45} F1={f1:.4f}{marker}")
        if baseline_runs:
            name = baseline_runs[0].data.tags.get("mlflow.runName", "baseline")
            print(f"  {name:<45} F1={baseline_f1:.4f}")
        print("="*55)
        return baseline_f1

    def _register_and_promote(self, run_id: str, version_name: str) -> object:
        model_uri  = f"runs:/{run_id}/distilbert-model"
        registered = mlflow.register_model(model_uri=model_uri, name=version_name)
        self.client.transition_model_version_stage(
            name                     = version_name,
            version                  = registered.version,
            stage                    = "Production",
            archive_existing_versions= True
        )
        return registered

    def run(self, train_metrics: dict = None) -> dict:
        experiment   = self._get_experiment()
        runs         = self._fetch_distilbert_runs(experiment.experiment_id)
        baseline_runs= self._fetch_baseline_runs(experiment.experiment_id)

        if not runs:
            print("No distilbert runs found to register.")
            return {}

        baseline_f1 = self._print_comparison(runs, baseline_runs)

        best_run    = runs[0]
        best_run_id = best_run.info.run_id
        best_f1     = best_run.data.metrics.get("f1_score", 0)
        best_name   = best_run.data.tags.get("mlflow.runName", best_run_id)

        print(f"\nBest model : {best_name}")
        print(f"Best F1    : {best_f1:.4f}")
        print(f"Improvement over baseline: +{best_f1 - baseline_f1:.4f}")

        print("Promoting to Production...")
        registered = self._register_and_promote(best_run_id, "fake-review-distilbert")
        print(f"Registered: fake-review-distilbert v{registered.version} → Production ✅")

        return {
            "best_run_id":               best_run_id,
            "best_run_name":             best_name,
            "best_f1":                   best_f1,
            "version":                   registered.version,
            "improvement_over_baseline": round(best_f1 - baseline_f1, 4),
        }


if __name__ == "__main__":
    ModelRegistry().run()
